...
public class LayoutCache
{
    ...
    private readonly List<LayoutCache>? _children;
    private readonly bool _isLast;

    public LayoutCache(Layout layout, LayoutDirection parentDirection = LayoutDirection.Row, bool isLast = false, int hierarchy = 0)
    {
        _layout = layout;
        ...
        if (layout.Children != null && layout.Children.Count > 0)
        {
            ...
            for (var i = 0; i <= lastIndex; i++)
            {
                ...
                _children.Add(new LayoutCache(child, layout.Direction, isLast, hierarchy + 1));
            }
        }
    }
        ...
        ExtractedRecord result)
    {
        ...
        if (array != null && array.Count > 0)
        {
            if (string.IsNullOrEmpty(_layout.DataField))
                throw new InvalidOperationException($"Repeat Layout expects {nameof(_layout.DataField)}");

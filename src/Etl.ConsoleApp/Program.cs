...
partial class Program
{
    static async Task Main(string[] arguments)
    {
        ...
        try
        {
            ...
            var builder = sp.GetRequiredService<EtlBuilder>()
               .SetConfig(args.Config)
               .SetConfig(args.ConfigFile)   
            ...
            var etl = builder.Build(args.DataFile);

            Console.WriteLine($"{DateTime.Now.Subtract(start)}");

            await etl.StartAsync(args.Take, args.Skip);
        }
        catch (Exception ex)
        {
            Exception? nested = ex;
            do
            {
                Console.Error.WriteLine(nested);
